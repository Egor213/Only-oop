...
{
    if (end.x < width && end.y < height && arr_cells[end.x][end.y].getPassability())
    {
        exit = end;
        getCell(end.x, end.y).setView('E');
        return true;
    }
    return false;
}
...
{
    if (start.x < width && start.y < height && arr_cells[start.x][start.y].getPassability())
    {
        this->start = start;
        getCell(start.x, start.y).setView('S');
        return true;
    }
    return false;
}
